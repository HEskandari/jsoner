...
func (extension *BinaryAsStringExtension) CreateEncoder(typ reflect2.Type) jsoner.ValEncoder {
	if typ == binaryType {
...
func (extension *BinaryAsStringExtension) CreateDecoder(typ reflect2.Type) jsoner.ValDecoder {
	if typ == binaryType {
		return &binaryAsStringCodec{}
	}
	return nil
...
func (codec *binaryAsStringCodec) Decode(ptr unsafe.Pointer, iter *jsoner.Iterator) {
...
func (codec *binaryAsStringCodec) Encode(ptr unsafe.Pointer, stream *jsoner.Stream) {
	newBuffer := writeBytes(stream.Buffer(), *(*[]byte)(ptr))
	stream.SetBuffer(newBuffer)
}

func readHex(iter *jsoner.Iterator, b1, b2 byte) byte {
	var ret byte
	if b1 >= '0' && b1 <= '9' {
		ret = b1 - '0'
	} else if b1 >= 'a' && b1 <= 'f' {
