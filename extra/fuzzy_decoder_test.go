...
func Test_any_to_string(t *testing.T) {
	...
	var val string
	should.Nil(api.UnmarshalFromString(`"100"`, &val))
	should.Equal("100", val)
	should.Nil(api.UnmarshalFromString(`"10.1"`, &val))
	should.Equal("10.1", val)
	should.NotNil(api.UnmarshalFromString("{}", &val))
	should.NotNil(api.UnmarshalFromString("[]", &val))
}
func Test_any_to_int64(t *testing.T) {
	t.Skip("decoder api should be removed")
	should := require.New(t)
	var val int64
	api := jsoner.DefaultAPI()
	should.Nil(api.UnmarshalFromString(`100`, &val))
	should.Equal(int64(100), val)
	should.Nil(api.UnmarshalFromString(`10`, &val))
	should.Equal(int64(10), val)
	should.Nil(api.UnmarshalFromString("", &val))
	should.Equal(int64(0), val)

	// bool part
	should.Nil(api.UnmarshalFromString(`false`, &val))
	should.Equal(int64(0), val)
	should.Nil(api.UnmarshalFromString(`true`, &val))
	...
	should.NotNil(api.UnmarshalFromString("[]", &val))
	// large float to int
	should.NotNil(api.UnmarshalFromString(`1234512345123451234512345.0`, &val))
}
...
func Test_any_to_uint64(t *testing.T) {
	...
	// TODO fix?
	should.NotNil(api.UnmarshalFromString(`-10`, &val))
	should.Equal(uint64(0), val)
	should.NotNil(api.UnmarshalFromString("{}", &val))
	should.NotNil(api.UnmarshalFromString("[]", &val))
	// large float to int
...
`
	...
	api := jsoner.DefaultAPI()
	err := api.UnmarshalFromString(jsonstr, &a)
	should := require.New(t)
	should.Nil(err)
}
...
func Test_null_to_string(t *testing.T) {
	...
	body := []byte(`null`)
	var message string
	err := jsoner.DefaultAPI().Unmarshal(body, &message)
	should.NoError(err)
}
