		...
		t.Run(tc.name, func(t *testing.T) {
			for _, target := range targets {
				t.Run(target.name, func(t *testing.T) {
					err := jsoner.DefaultAPI().Unmarshal(tc.data, target.new())
...
func Test_nested(t *testing.T) {
	iter := jsoner.ParseString(jsoner.DefaultAPI(), `{"hello": [{"world": "value1"}, {"world": "value2"}]}`)
	l1 := Level1{}
...
func Benchmark_jsoner_nested(b *testing.B) {
	for n := 0; n < b.N; n++ {
		iter := jsoner.ParseString(jsoner.DefaultAPI(), `{"hello": [{"world": "value1"}, {"world": "value2"}]}`)
		l1 := Level1{}
		for l1Field := iter.ReadObject(); l1Field != ""; l1Field = iter.ReadObject() {
...
func readLevel1Hello(iter *jsoner.Iterator) []Level2 {
