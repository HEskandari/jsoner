...
func Test_two_elements(t *testing.T) {
	should := require.New(t)
	iter := jsoner.ParseString(jsoner.DefaultAPI(), `[1,2]`)
	should.True(iter.ReadArray())
...
func Test_write_array(t *testing.T) {
	should := require.New(t)
	buf := &bytes.Buffer{}
	stream := jsoner.NewStream(jsoner.Config{IndentionStep: 2}.Froze(), buf, 4096)
...
func Test_encode_byte_array(t *testing.T) {
	...
	bytes, err = jsoner.DefaultAPI().Marshal([]byte{1, 2, 3})
	should.Nil(err)
	should.Equal(`"AQID"`, string(bytes))
}
...
func Test_encode_nil_byte_array(t *testing.T) {
	...
	bytes, err = jsoner.DefaultAPI().Marshal(nilSlice)
	should.Nil(err)
	should.Equal(`null`, string(bytes))
}
...
func Test_decode_byte_array_from_base64(t *testing.T) {
	...
	err = jsoner.DefaultAPI().Unmarshal([]byte(`"AQID"`), &data)
	should.Nil(err)
	should.Equal([]byte{1, 2, 3}, data)
}
...
func Test_decode_byte_array_from_base64_with_newlines(t *testing.T) {
	...
	should.Nil(err)
	should.Equal([]byte{1, 2, 3}, data)
	err = jsoner.DefaultAPI().Unmarshal([]byte(`"A\rQ\nID"`), &data)
	should.Nil(err)
	should.Equal([]byte{1, 2, 3}, data)
}
...
func Test_decode_byte_array_from_array(t *testing.T) {
	...
	should.Nil(err)
	should.Equal([]byte{1, 2, 3}, data)
	err = jsoner.DefaultAPI().Unmarshal([]byte(`[1,2,3]`), &data)
	should.Nil(err)
	should.Equal([]byte{1, 2, 3}, data)
}
...
func Test_decode_slice(t *testing.T) {
	...
	jsoner.DefaultAPI().UnmarshalFromString(`["hello", "world"]`, &slice)
...
func Test_decode_large_slice(t *testing.T) {
	should := require.New(t)
	slice := make([]int, 0, 1)
	jsoner.DefaultAPI().UnmarshalFromString(`[1,2,3,4,5,6,7,8,9]`, &slice)
	should.Equal([]int{1, 2, 3, 4, 5, 6, 7, 8, 9}, slice)
}

func Benchmark_jsoner_array(b *testing.B) {
	b.ReportAllocs()
