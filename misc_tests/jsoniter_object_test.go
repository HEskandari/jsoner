...
func Test_empty_object(t *testing.T) {
	...
	should.Equal("", field)
	iter = jsoner.ParseString(jsoner.DefaultAPI(), `{}`)
	iter.ReadObjectCB(func(iter *jsoner.Iterator, field string) bool {
...
func Test_one_field(t *testing.T) {
	should := require.New(t)
	iter := jsoner.ParseString(jsoner.DefaultAPI(), `{"a": "stream"}`)
	field := iter.ReadObject()
	...
	should.Equal("", field)
	iter = jsoner.ParseString(jsoner.DefaultAPI(), `{"a": "stream"}`)
	should.True(iter.ReadObjectCB(func(iter *jsoner.Iterator, field string) bool {
		should.Equal("a", field)
		iter.Skip()
		return true
	}))
...
func Test_two_field(t *testing.T) {
	should := require.New(t)
	iter := jsoner.ParseString(jsoner.DefaultAPI(), `{ "a": "stream" , "c": "d" }`)
	field := iter.ReadObject()
	should.Equal("a", field)
	value := iter.ReadString()
	should.Equal("stream", value)
	...
	field = iter.ReadObject()
	should.Equal("", field)
	iter = jsoner.ParseString(jsoner.DefaultAPI(), `{"field1": "1", "field2": 2}`)
...
func Test_unmarshal_into_existing_value(t *testing.T) {
	...
	obj.Field2 = &m
	cfg := jsoner.Config{UseNumber: true}.Froze()
