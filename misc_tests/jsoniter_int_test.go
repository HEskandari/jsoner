...
func Test_read_uint64_invalid(t *testing.T) {
	...
	iter := jsoner.ParseString(jsoner.DefaultAPI(), ",")
	iter.ReadUint64()
...
func Test_read_int32_array(t *testing.T) {
	...
	input := `[123,456,789]`
	val := make([]int32, 0)
	jsoner.DefaultAPI().UnmarshalFromString(input, &val)
...
func Test_wrap_int(t *testing.T) {
	should := require.New(t)
	str, err := jsoner.DefaultAPI().MarshalToString(jsoner.WrapInt64(100))
	should.Nil(err)
...
func Test_float_as_int(t *testing.T) {
	should := require.New(t)
	var i int
	should.NotNil(jsoner.DefaultAPI().Unmarshal([]byte(`1.1`), &i))
}
...
func TestIterator_ReadInt_chunkedInput(t *testing.T) {
	...
	for data.chunkSize = 3; data.chunkSize <= len(data.data); data.chunkSize++ {
		...
		iter := jsoner.Parse(jsoner.DefaultAPI(), data, data.chunkSize)
		i := 0
		for iter.ReadArray() {
...
func jsonFloatIntArray(t *testing.T, numberOfItems int) []byte {
	t.Helper()
	numbers := make([]jsoner.Any, numberOfItems)
	for i := range numbers {
		switch i % 2 {
		case 0:
			numbers[i] = jsoner.WrapFloat64(0.1)
		default:
			numbers[i] = jsoner.WrapInt64(0)
		}
	}

	fixture, err := jsoner.FastestAPI().Marshal(numbers)
	if err != nil {
		panic(err)
	}
...
func Benchmark_jsoner_encode_int(b *testing.B) {
	stream := jsoner.NewStream(jsoner.DefaultAPI(), ioutil.Discard, 64)
	for n := 0; n < b.N; n++ {
		stream.Reset(nil)
...
func Benchmark_jsoner_int(b *testing.B) {
