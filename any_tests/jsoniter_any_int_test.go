...
func Test_read_any_to_int(t *testing.T) {
	...
	for k, v := range intConvertMap {
		any := jsoner.DefaultAPI().Get([]byte(k))
...
func Test_int_lazy_any_get(t *testing.T) {
	should := require.New(t)
	any := jsoner.DefaultAPI().Get([]byte("1234"))
	// panic!!
	//should.Equal(any.LastError(), io.EOF)
	should.Equal(jsoner.InvalidValue, any.Get(1, "2").ValueType())
}