...
func Test_read_object_as_any(t *testing.T) {
	...
	any := jsoner.DefaultAPI().Get([]byte(`{"a":"stream","c":"d"}`))
	...
	should.Equal(jsoner.ObjectValue, any.ValueType())
...
func Test_object_lazy_any_get_all(t *testing.T) {
	should := require.New(t)
	any := jsoner.DefaultAPI().Get([]byte(`{"a":[0],"stream":[1]}`))
...
func Test_object_lazy_any_get_invalid(t *testing.T) {
	...
	should.Equal(jsoner.InvalidValue, any.Get("a", "stream", "c").ValueType())
	should.Equal(jsoner.InvalidValue, any.Get(1).ValueType())
}

func Test_wrap_map_and_convert_to_any(t *testing.T) {
	should := require.New(t)
	any := jsoner.Wrap(map[string]interface{}{"a": 1})
	should.True(any.ToBool())
	should.Equal(0, any.ToInt())
...
func Test_wrap_object_and_convert_to_any(t *testing.T) {
	...
	any := jsoner.Wrap(TestObject{"hello", "world"})
...
func Test_any_within_struct(t *testing.T) {
	...
	obj := TestObject{}
	err := jsoner.DefaultAPI().UnmarshalFromString(`{"Field1": "hello", "Field2": [1,2,3]}`, &obj)
	should.Nil(err)
...
func Test_object_wrapper_any_get_all(t *testing.T) {
	...
	any := jsoner.Wrap(TestObject{[]int{1, 2}, []int{3, 4}})
