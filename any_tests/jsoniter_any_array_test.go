...
func Test_read_empty_array_as_any(t *testing.T) {
	...
	any := api.Get([]byte("[]"))
	should.Equal(jsoner.ArrayValue, any.Get().ValueType())
	should.Equal(jsoner.InvalidValue, any.Get(0.3).ValueType())
	should.Equal(0, any.Size())
	should.Equal(jsoner.ArrayValue, any.ValueType())
...
func Test_wrap_array_and_convert_to_any(t *testing.T) {
	should := require.New(t)
	any := jsoner.Wrap([]int{1, 2, 3})
	any2 := jsoner.Wrap([]int{})
...
func Test_array_lazy_any_get_invalid(t *testing.T) {
	...
	api := jsoner.DefaultAPI()
	any := api.Get([]byte("[]"))
	should.Equal(jsoner.InvalidValue, any.Get(1, 1).ValueType())
...
func Test_invalid_array(t *testing.T) {
	...
	api := jsoner.DefaultAPI()
	any := api.Get([]byte("["), 0)
	should.Equal(jsoner.InvalidValue, any.ValueType())
}