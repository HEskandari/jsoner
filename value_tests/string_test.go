...
func Test_read_string(t *testing.T) {
	...
	for _, input := range badInputs {
		...
		testReadString(t, input, "", true, "jsoner.Unmarshal", jsoner.DefaultAPI().Unmarshal)
		testReadString(t, input, "", true, "jsoner.ConfigCompatibleWithStandardLibrary.Unmarshal", jsoner.CompatibleAPI().Unmarshal)
	}
	...
	for _, tc := range goodInputs {
		...
		testReadString(t, tc.input, tc.expectValue, false, "jsoner.Unmarshal", jsoner.DefaultAPI().Unmarshal)
		testReadString(t, tc.input, tc.expectValue, false, "jsoner.ConfigCompatibleWithStandardLibrary.Unmarshal", jsoner.CompatibleAPI().Unmarshal)
	}
}
