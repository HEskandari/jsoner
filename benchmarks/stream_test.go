...
func Benchmark_stream_encode_big_object(b *testing.B) {
	...
	var stream = jsoner.NewStream(jsoner.DefaultAPI(), &buf, 100)
	for i := 0; i < b.N; i++ {
		buf.Reset()
		stream.Reset(&buf)
		encodeObject(stream)
...
func TestEncodeObject(t *testing.T) {
	var stream = jsoner.NewStream(jsoner.DefaultAPI(), nil, 100)
	encodeObject(stream)
	if stream.Error != nil {
	...
	if err := jsoner.DefaultAPI().Unmarshal(stream.Buffer(), &m); err != nil {
		t.Errorf("error unmarshaling a test object: %+v", err)
		return
	}
}

func encodeObject(stream *jsoner.Stream) {
