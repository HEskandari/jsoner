...
*/
func Benchmark_jsoner_large_file(b *testing.B) {
	b.ReportAllocs()
	for n := 0; n < b.N; n++ {
		file, _ := os.Open("/tmp/large-file.json")
		iter := jsoner.Parse(jsoner.DefaultAPI(), file, 4096)
		count := 0
		iter.ReadArrayCB(func(iter *jsoner.Iterator) bool {
			// Skip() is strict by default, use --tags jsoner-sloppy to skip without validation
			iter.Skip()
			count++
			return true
		})
