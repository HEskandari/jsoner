	...
	for _, testCase := range testCases {
		...
		t.Run(valType.String(), func(t *testing.T) {
			...
			for i := 0; i < 100; i++ {
				...
				jbIter, err := jsoner.CompatibleAPI().Marshal(before)
				if err != nil {
					t.Fatalf("failed to marshal with jsoner: %v", err)
				}
				if len(strings.TrimSpace(string(jbIter))) == 0 {
					t.Fatal("jsoner marshal produced empty result and no error")
				}
				...
				err = jsoner.CompatibleAPI().Unmarshal(jbIter, afterIterPtr)
				if err != nil {
					t.Fatalf("failed to unmarshal with jsoner: %v\nvia:\n    %s",
						err, indent(jbIter, "    "))
				}
